...
class UnresolvedDialog(tk.Toplevel):
    ...
    def __init__(self, parent, unresolved):
        ...
        for key, info in sorted(unresolved.items(), key=lambda kv: -kv[1]["count"]):
            ...
            # AI taklifi kelganda shu yerda ko'rsatiladi (bo'sh bo'lsa
            # ko'rinmaydi). Taklif faqat maslahat — yozilgan qiymatni
            # foydalanuvchi istagancha o'zgartirishi mumkin.
        ...
        self._start_ai_suggestions()

        self.protocol("WM_DELETE_WINDOW", self._on_cancel)

    # ------------------------------------------------ AI takliflari
    def _start_ai_suggestions(self):
        """Fon oqimida Gemini'dan guruh takliflarini so'raydi. Bu faqat
        yordam — javob kelmasa ham oyna avvalgidek ishlayveradi."""
        if not ai_helper.is_available() or not self._row_order:
            return
        self._ai_status.configure(text="AI takliflari so'ralmoqda...")

        items = [self.result_entries[k][0] for k in self._row_order]
        groups = (
            {v for v in categorize.KNOWN_VENDOR_INN.values()}
            | {cat for _, cat in categorize.PURPOSE_FIRST_RULES}
            | {cat for _, cat, _ in categorize.TEXT_RULES}
        )

        def worker():
            result = ai_helper.suggest(items, groups)
            self.after(0, lambda: self._apply_ai_suggestions(result))

        threading.Thread(target=worker, daemon=True).start()

    def _apply_ai_suggestions(self, suggestions):
        """Takliflarni maydonlarga yozadi. Foydalanuvchi allaqachon o'zi
        biror narsa yozgan bo'lsa — tegilmaydi."""
